	...
	if row_index == 0:
		...
		for cell_index, cell in enumerate(row):
			...
			if "bq.tempstorage" in cell_heading:
				cell_heading = data_headings[cell_index].replace("_", ".")
			if "column.comparision" in cell_heading:
				cell_heading = data_headings[cell_index].replace(".", "_")
			if "column.comparision.tgt" in cell_heading:
				cell_heading = data_headings[cell_index].replace(".", "_")
			if "null.id" in cell_heading:
				cell_heading = data_headings[cell_index].replace(".", "_")
			if "merge.condition" in cell_heading:
				cell_heading = data_headings[cell_index].replace(".", "_")
			if "join.condition" in cell_heading:
				cell_heading = data_headings[cell_index].replace(".", "_")
			if "src.fields" in cell_heading:
				cell_heading = data_headings[cell_index].replace(".", "_")
